...
function createCodeEditorBehaviors(config) {
  return [behavior_core.defineBehavior({
    ...
    guard: ({
      ...
      const isMoveUpShortcut = behavior_core.isHotkey(config.moveBlockUpShortcut, event.originEvent), firstBlock = selector_isAtTheStartOfBlock.getFirstBlock(snapshot), selectedBlocks = selector_isAtTheStartOfBlock.getSelectedBlocks(snapshot), blocksAbove = firstBlock?.node._key !== selectedBlocks[0]?.node._key;
    ...
    guard: ({
      ...
      const isMoveDownShortcut = behavior_core.isHotkey(config.moveBlockDownShortcut, event.originEvent), lastBlock = selector_isAtTheStartOfBlock.getLastBlock(snapshot), selectedBlocks = selector_isAtTheStartOfBlock.getSelectedBlocks(snapshot), blocksBelow = lastBlock?.node._key !== selectedBlocks[selectedBlocks.length - 1]?.node._key;
...
function createLinkBehaviors(config) {
  const pasteLinkOnSelection = behavior_core.defineBehavior({
    ...
    guard: ({
      ...
      const selectionCollapsed = selector_isAtTheStartOfBlock.isSelectionCollapsed(snapshot), text = event.originEvent.dataTransfer.getData("text/plain"), url = looksLikeUrl(text) ? text : void 0, annotation = url !== void 0 ? config.linkAnnotation?.({
        url,
