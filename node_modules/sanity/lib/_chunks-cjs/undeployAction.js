...
async function undeployCoreAppAction(_, context) {
  ...
  const appId = cliConfig && "__experimental_coreAppConfiguration" in cliConfig ? cliConfig.__experimental_coreAppConfiguration?.appId : void 0;
  if (!appId) {
    spinner.fail(), output.print("No Core application ID provided."), output.print("Please set appId in `__experimental_coreAppConfiguration` in sanity.cli.js or sanity.cli.ts."), output.print("Nothing to undeploy.");
    return;
  }
  ...
  if (output.print(""), !!await prompt.single({
    type: "confirm",
    default: !1,
    message: `This will undeploy ${chalk.yellow(userApplication.appHost)} and make it unavailable for your users.
