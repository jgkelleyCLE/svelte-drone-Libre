...
async function deployAppAction(args, context) {
  const {
    ...
    flags,
    cliConfig
  }), installedSanityVersion = await helpers.getInstalledSanityVersion(), appId = cliConfig && "__experimental_coreAppConfiguration" in cliConfig && cliConfig.__experimental_coreAppConfiguration?.appId, client = apiClient({
    requireUser: !0,
    requireProject: !1
...
`);
  ...
  try {
    ...
    appId ? userApplication = await helpers.getOrCreateUserApplicationFromConfig({
      ...configParams,
      appId
    }) : userApplication = await helpers.getOrCreateCoreApplication(configParams);
  } catch (err) {
    if (err.message) {
...
Add ${chalk.cyan(`appId: '${userApplication.id}'`)}`), output.print("to __experimental_coreAppConfiguration in sanity.cli.js or sanity.cli.ts"), output.print("to avoid prompting on next deploy."));
  } catch (err) {
