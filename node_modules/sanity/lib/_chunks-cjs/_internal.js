...
function determineIsCoreApp(cliConfig) {
  return !!(cliConfig && "__experimental_coreAppConfiguration" in cliConfig);
}
const helpText$l = `
Options
  --source-maps Enable source maps for built bundles (increases size of bundle)
...
`, deployCommand = {
  ...
  action: async (args, context) => {
    let mod;
    return determineIsCoreApp(context.cliConfig) ? mod = await Promise.resolve().then(function() {
...
async function getDevAction(context) {
  return determineIsCoreApp(context.cliConfig) ? (await Promise.resolve().then(function() {
...
exports.determineIsCoreApp = determineIsCoreApp;
